...
ability(:,1) = a0;
employed(:,1) = 0;
olf(:,1)      = 1;   % <<< CHANGED: newborns start OLF, not unemployed
married(:,1)  = 0;
has_kids(:,1) = 0;
wage(:,1)     = 0;
...
for i = 1:N
    if female(i)
        value_now(i,1) = valueFunc.single_female.olf_nokid(ability(i,1));  % <<< CHANGED
    else
        value_now(i,1) = valueFunc.single_male.olf_nokid(ability(i,1));    % <<< CHANGED
    end
end
...
h         = repmat(dt_vec,N,1);
U_meet(:,2:T_total) = U_meet(:,2:T_total).*(U_death(:,1:T_total-1) > exp(-param.zeta .* h(:,1:T_total-1)));
...
parfor i = 1:N
    ...
    for t = 2:T_total
        ...
        if (u_die > exp(-param.zeta * h))
            ...
            m(t) = 0; e(t) = 0; o(t) = 1;  % <<< CHANGED: newborns start unemployed
            w(t) = 0; k(t) = 0;
            ...
            if isF
                ...
                value_now_i = valueFunc.single_female.olf_nokid(a(t));  % <<< CHANGED
            else
                a(t) = clip(randn*sqrt(param.pm.aergvar) + param.pm.aergmean, param.a.min, param.a.max);
                value_now_i = valueFunc.single_male.olf_nokid(a(t));    % <<< CHANGED
            end
            V(t) = value_now_i;
        end
